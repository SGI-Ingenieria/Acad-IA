...
import { generateInitialChatTitle } from '../_shared/chat-title.ts'
import {
...
// Preflight CORS
app.options('*', (_c) => preflightResponse())
...
function readStructuredOutput(result: any): Record<string, unknown> {
  ...
  if (typeof result?.outputText === 'string') {
    return parseOpenAIJsonText(result.outputText) as Record<string, unknown>
  }
  throw new HttpError(
    502,
    'ai_error',
...
app.post(`${prefix}/plan/conversations`, async (c) => {
  try {
    const user = await requireAuthenticatedUser(c.req.raw, {
      missingAuthorizationMessage: 'Missing Authorization header',
      missingAuthorizationCode: 'missing_auth',
      invalidAuthorizationMessage: 'Invalid or expired token',
      invalidAuthorizationCode: 'invalid_auth',
    })
  ...
  try {
    ...
    if (editFields.length === 0) {
      ...
      if (intent.type === 'accion') {
        ...
        const actionRecord = actionOutput as Record<string, unknown>
            ...
            : intent.accion === 'eliminar_linea'
              ...
              : intent.accion === 'asignar_asignatura'
                ...
                : intent.accion === 'cambio_ciclo'
                  ...
                  : (
                      (actionRecord.sugerencias as Array<
                        Record<string, unknown>
                      >) ?? []
                    ).map((proposal) => ({ tipo: 'asignatura', ...proposal }))
        const actionResponse =
          intent.accion === 'proponer_linea'
            ? `Propongo la línea curricular “${String(
                actionRecord.nombre ?? intent.nombre ?? '',
              )}”. ${String(actionRecord.justificacion ?? '')}`
            : intent.accion === 'eliminar_linea'
              ? 'error' in actionOutput
                ? String(actionOutput.error)
                : `Encontré la línea curricular “${String(
                    actionOutput.linea_nombre ?? intent.lineaNombre ?? '',
              ...
              : intent.accion === 'asignar_asignatura'
                ...
                : intent.accion === 'cambio_ciclo'
                  ? 'error' in actionOutput
                    ? String(actionOutput.error)
                    : `Preparé el cambio de “${String(
                        actionOutput.asignatura_nombre ?? '',
                      )}” al ${String(
                        actionOutput.numero_ciclo ?? '',
      ...
      editFields = intent.type === 'edicion' ? intent.campos : []
    }
    ...
    const documentSupabase = getServiceRoleClient()
    ...
    const documentReferenceQuery =
      frozenDocumentReferences?.query ?? promptWithMentions

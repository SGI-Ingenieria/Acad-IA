...
export function cachePath(
  ...
  return `cache/${CACHE_VERSION}/${asignaturaId}/${format}/${cacheKey}.${
...
export async function computeCacheKey(
  ...
  const payload = `${CACHE_VERSION}:${asignaturaId}:${format}:${
    sorted.join(",")
  }`;
...
function localSupabasePublicUrl(): string | undefined {
  ...
  try {
    const internalBase = new URL(internalBaseUrl);
    if (internalBase.hostname !== "kong") return undefined;
    const port = Deno.env.get("SUPABASE_INTERNAL_HOST_PORT") ?? "54321";
...
export function clientFileName(
  ...
  const prefijo = format === "scorm_1_2"
    ? "scorm"
    : format === "html_bundle"
    ? "html"
    : format === "pptx_bundle"
  ...
  const slugContenido = objetos.length === 1

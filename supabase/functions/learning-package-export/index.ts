...
function indexarContenido(contenido: unknown): NombresContenido {
  ...
  lista.forEach((unidadRaw, unidadIdx) => {
    if (
      !unidadRaw || typeof unidadRaw !== "object" || Array.isArray(unidadRaw)
    ) {
      return;
    }
    const unidad = unidadRaw as Record<string, unknown>;
    const numero = typeof unidad.unidad === "number"
      ? unidad.unidad
      : unidadIdx + 1;
    const titulo = typeof unidad.titulo === "string" && unidad.titulo
...
function buildContext(
  ...
  return {
    ...
    asignaturaCodigo: typeof asignatura.codigo === "string"
      ? asignatura.codigo
      : null,

...
function getAction(req: Request, body: JsonRecord) {
  ...
  const pathAction = ownIndex >= 0 && segments[ownIndex + 1]
    ? segments[ownIndex + 1]
    : null;
...
async function checkSupabaseConnectivity(client: SupabaseClientAny) {
  ...
  try {
    ...
    return {
      ...
      serverTime: data && typeof data === "object" && "server_time" in data
...
function openAIWebhookUrl() {
  ...
  return `https://platform.openai.com/settings/${
    encodeURIComponent(
      projectId,
    )
  }/webhooks`;
...
async function checkOpenAIHealth() {
  const env = {
    ...
    healthcheckModel: readEnv("OPENAI_HEALTHCHECK_MODEL") || "gpt-5-nano",
  };
    ...
    if (!identity.response.ok) {
      return {
        ...
        identity: {
          ...
          latencyMs: identity.latencyMs,
          valid: false,
          message: identity.body.slice(0, 220) ||
    ...
    if (!models.response.ok) {
      return {
        ...
        models: {
          ...
          latencyMs: models.latencyMs,
          reachable: false,
          message: models.body.slice(0, 220) ||
...
async function runOpenAIBackgroundTest(req: Request) {
  ...
  try {
    ...
    await updateTestRun(serviceClient, run.id, {
      ...
      metadata: {
        source: "observability-dashboard",
        openai_status: typeof response.status === "string"
          ? response.status
          : "queued",
      },
    });
...
async function readRecentObservabilityRows(client: SupabaseClientAny) {
  const [eventsResult, runsResult] = await Promise.all([
    client
      .from("observability_webhook_events")
      .select(
        "id,event_id,event_type,openai_response_id,test_run_id,received_at,signature_valid,processing_status,processing_error",
      )
  ...
  return {
    ...
    status: eventsResult.error || runsResult.error
...
async function snapshot(req: Request) {
  ...
  try {
    ...
    serviceClientError = error instanceof Error
      ? error.message
      : "No esta configurado el token de servicio de Supabase.";
  }

  const [supabase, edgeFunctions, openai, migrations, recent] = await Promise
    .all([
      checkSupabaseConnectivity(createAnonClient(auth.token)),
      buildEdgeFunctionsHealth(auth.token),
      checkOpenAIHealth(),

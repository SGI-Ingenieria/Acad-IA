...
Deno.test(
  ...
  () => {
    ...
    assertEquals(
      resolveSpeechModel("modelo-voz-interno"),
      "modelo-voz-interno",
    );
  },
);

Deno.test("el chat reconoce los bloques de conocimiento como líneas curriculares", () => {
  const prompt = buildIntentSystemPrompt({
    entityType: "plan",
    entityJson: {
      lineas_curriculares: [
        { nombre: "Matemáticas y ciencias básicas", orden: 2 },
      ],
    },
    editableFields: [],

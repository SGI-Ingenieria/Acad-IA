...
Deno.test(
  ...
  () => {
    const signedUrl =
      "http://kong:8000/storage/v1/object/sign/learning-packages/cache/v1/a/html_bundle/file.zip?token=abc&download=file.zip";
...
Deno.test(
  "clientSignedUrl usa el puerto publico de Supabase local",
  () => {
    const originalSupabaseUrl = Deno.env.get("SUPABASE_URL");
    const originalPublicUrl = Deno.env.get("SUPABASE_PUBLIC_URL");
    const originalExternalUrl = Deno.env.get("API_EXTERNAL_URL");

-- Sincroniza la RPC del catálogo con el parámetro de orden usado por el
-- frontend. Se elimina la firma anterior para evitar sobrecargas ambiguas en
-- PostgREST cuando los parámetros opcionales se omiten.
...
as $$
  ...
  filtered as (
    ...
    where public.authz_can_list_plan_catalog_for_facultad(c.facultad_id)
      and (
        n.search_term = ''
        or pe.nombre_search ilike '%' || n.search_term || '%'
      )
  ...
  order by
    case
      when n.safe_sort = 'actualizado_desc'
        then (filtered.pe).actualizado_en
    end desc nulls last,
    case
      when n.safe_sort = 'nombre_asc'
        then lower(coalesce((filtered.pe).nombre, ''))
    end asc nulls last,
    case
      when n.safe_sort = 'nombre_desc'
        then lower(coalesce((filtered.pe).nombre, ''))
    end desc nulls last,
    case
      when n.safe_sort = 'creado_desc'
        then (filtered.pe).creado_en
    end desc nulls last,
    (filtered.pe).id
...
grant all on function public.planes_catalogo_buscar(
  text,
  uuid,
  uuid,
  uuid,

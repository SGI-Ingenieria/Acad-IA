-- Evita evaluar la autorización completa una vez por asignatura. El acceso al
-- plan es estable durante la consulta, por lo que se calcula una sola vez por
-- plan y las agregaciones JSON se construyen únicamente para la página pedida.
...
-- El selector de estado sólo necesita ids. Evita descargar hasta cien planes
-- completos, sus carreras, facultades, estructuras y estados para deduplicarlos
-- después en el navegador.
create or replace function public.planes_catalogo_estados_disponibles(
  p_facultad_id uuid default null,
  p_carrera_id uuid default null,
...
alter function public.planes_catalogo_estados_disponibles(

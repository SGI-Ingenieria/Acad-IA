-- Los documentos suelen expresar el inicio como periodo (p. ej. «2016-I»),
-- mientras que los planes almacenan una fecha de calendario. La conversión se
-- hace aquí, en el límite de persistencia, para que un valor documental nunca
-- invalide la aplicación atómica de la importación.
create or replace function public.normalizar_fecha_importacion_documental(
  p_valor text
)
returns date
language plpgsql
...
grant execute on function public.normalizar_fecha_importacion_documental(text) to authenticated;

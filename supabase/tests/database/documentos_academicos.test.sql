...
-- El pipeline de extracción propia fue retirado: el retrieval usa la cascada
-- de vector stores de OpenAI.
select hasnt_function(
  'public', 'search_authorized_chunks',
  'la búsqueda de chunks propia fue retirada'
);
select hasnt_function(
  'public', 'finalizar_extraccion_openai_documental',
  'la finalización de extracciones de OpenAI fue retirada'
);
select hasnt_function(
  'public', 'registrar_webhook_documental',
  'el webhook de extracción documental fue retirado'
);
select ok(
  has_function_privilege(
    'service_role',
...
select hasnt_function(
  ...
  'la finalización de indexación propia fue retirada'
);
select ok(
  exists (
...
select is(
  (
    select count(*)::integer from pgmq.list_queues()
    where queue_name in ('file-hashing', 'file-cleanup', 'openai-sync', 'vs-warmup')
  ),
  4,
  'las colas documentales vigentes son durables'
);
select ok(
  exists (
...
-- Los tipos de trabajo del pipeline retirado se rechazan explícitamente.
select throws_ok(
  $$ select public.encolar_trabajo_ingesta_documental(
       'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa',
       null, null, 'chunk', 'chunk:test-retirado', '{}'::jsonb
     ) $$,
  '55000',
  'El pipeline de extracción propia fue retirado; el retrieval usa la cascada de OpenAI.',
  'los trabajos de chunking ya no pueden encolarse'
);
